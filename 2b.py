raw_input = """6-10 s: snkscgszxsssscss
6-7 b: bbbbbxkb
2-4 n: nnnjn
1-2 j: jjjj
5-9 z: jgzzzqhbj
...
#Define the list, that will be a list of dictionaries
passwords_and_policies= []

#Loop through the list of strings, splitting it up and make each line a dictionary

i = 0

while i < len(split_input):

    x = list(split_input[i].split())

    #Define Temporary Dictionary to be used for each set of min, max and char
    thisdict= {}

    # define min and max and add to dictionary as an int
    y = x[0].split("-")
    thisdict.update({"min" : int(y[0])})
    thisdict.update({"max" : int(y[1])})

    #Define "char" and add it to the dictionary
    z = x[1].split(":")
    thisdict.update({"char" : z[0]})

    #Define "password" and add it to the dictionary
    a = x[2]
    thisdict.update({"password" : a})

    #add this dictionary to the list of "Passwords and policies
    passwords_and_policies.append(thisdict)
    i+=1

#clear temporary variables
del x, y, z, a, thisdict

#initialise the counter
counter = 0

#Loop through "passwords_and_policies" and count each time the policies are followed correctly
for y in range(0, len(passwords_and_policies)):

    #extract the relevant numbers from the dictionary
    x = passwords_and_policies[y]
    pos1_number = x["min"]
    pos2_number = x["max"]
    character = x["char"]
    password = x["password"]

    #add to counter if conditions are met
    if password[pos1_number-1] == character  and password[pos2_number-1] == character:
        pass
    elif password[pos1_number-1] == character or password[pos2_number-1] == character:
        counter +=1

del i, x, pos1_number, pos2_number, character,password

print('There are ' + str(counter) + ' good solutions ' + 'out of ' + str(len(passwords_and_policies)) + ' total for AOC 2020 Day 2, part b.')
